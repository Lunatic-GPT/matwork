function Allineate_afni_e2a_Trilinear(master_set,mat1D,input,info_input)

% Use T1_al_e2a_only_mat.aff12.1D; 
% However, for plumb orientation this file is not
...
%}
             ...
             xyz_new=ijk2real_inv*(pos3-ijk2real(:,4))+1;
             
             xyz_l=floor(xyz_new);
             xyz_u=xyz_l+1;
             
             xyz=[xyz_l(:),xyz_u(:)];
             if any(xyz_l<1)||any(xyz_u>size(d)')
                 continue;
             end
             
             d_out(i,j,k)=trilinear_interpolation(xyz_new,xyz,d(xyz_l(1):xyz_u(1),xyz_l(2):xyz_u(2),xyz_l(3):xyz_u(3)));
...
opt.Prefix=[prefix,'_ToT1_Trilin'];
