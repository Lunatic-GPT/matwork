function gmems_csreconbr(a,format)
...
data=reshape(data,[mtrx(1),nch,ne,ns,mtrx(2)/reduction,reduction*NR]);
...
z2=zeros([mtrx',ns,ne,reduction*NR,nch]);
...
switch method
    case 'nesta'
        ...
        %  recon(k,:,i,:,j)=ch_scl(j)*ktFOCUSS_new(z2(k,:,i,:,j),[],[],3,m2(k,:,i,:),0.5,lambda,60,2,1);  
          recon(k,:,i,ie,:,j)=ch_scl(j)*ktNESTA_UP(fz2(k,:,i,ie,:,j),m2(k,:,i,:));
        
          end
          tmp=reshape(recon(:,:,i,ie,:,j),[64,64,1,size(recon,5)]);
         write_afni(abs(tmp),sprintf('scan11_slice%d_ch%d_echo%d',i,j,ie));
        ...
        for j=1:nch
         for i=1:size(z2,3)
                 for ie=1:ne
                  ...
                  recon(:,:,i,ie,:,j)=ch_scl(j)*ktFOCUSS_new(z2(:,:,i,ie,:,j),[],[],3,m2(:,:,i,:),0.5,lambda,60,2,1);  
                 end
          end
        end
    case 'lowk'
  ...
  %   recon=flipdim(recon,2);
    case 'ktfklt'
        ...
        a=BrikLoad('11_ktf_0+orig');
        
        for j=1:nch
            for ie=1:ne
                
             for i=1:size(z2,3)  
                 prev=a(:,:,i,(j-1)*1280+1:j*1280);
                 prev=prev(:,:,1,ie:4:end);
              fprintf('Slice %d\n',i);
              for iklt=1:3
               recon(:,:,i,ie,:,j)=ktFOCUSS_KLT_new(z2(:,:,i,ie,:,j),prev,[],[],m2(:,:,i,:),0.5,0.01,100,2,1);  
               prev=recon(:,:,i,ie,:,j);
              end
             end
            end
...
sz=size(recon);
recon=reshape(recon,[sz(1:3),sz(4)*sz(5)*sz(6)]);
   
if format=='a'
 write_afni(abs(recon),[a,'_',method,'_',num2str(lambda)]);
elseif format=='s'
