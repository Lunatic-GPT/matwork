function mask=cstable_Dti(N,nTR,nb,reduction,ns,nc,suffix)
%cstable_gems_mTR(N,nTRcs,nb,reduction,ns,nc,suffix)
% N: number of phase encoding lines (before reduction)
% nTRcs: number of TRs
% reduction: reduction factor
% ns: number of slices
% nb: number of b values (after undersampling q space)
...
npe = reshape(npe,[nout+nc,ns,nb,nTR]);
npe = permute(npe,[2,3,1,4]);

%%
fname = sprintf('petable_R%d_N%d_nTR%d_nb%d_ns%d_nc%d_%s',reduction,N,nTR,nb,ns,nc,suffix);
