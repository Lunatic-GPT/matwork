function recon=ktFOCUSS_new(y, y_true,ind_sparse,num_low_phase, M,factor,lambda, Minner, Mouter, pred)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%% Hong's k-t FOCUSS code for cartesian trajectory %%%%%%%%%%%%%%%%%%%%%
% recon=ktFOCUSS_new(y, y_true,ind_sparse,num_low_phase, M,factor,lambda,
% Minner, Mouter, pred)
% y: zero-padded measurements on k-t space, dimension=(kx, ky, time);
...
%    1<=ky<=4, end-3<=ky<=end : M(kx, ky, time)=1 
%    
...
y=circshift(y,[floor(size(y,1)/2),size(y,2)/2,0,0]);
y=squeeze(y);
if ndims(y)==2
    y=reshape(y,[1,size(y,1),size(y,2)]);
end
y=permute(y,[2,1,3]);
M=circshift(M,[floor(size(M,1)/2),size(M,2)/2,0,0]);
M=squeeze(M);
if ndims(M)==2
...
%% added by XZ
recon=circshift(recon,[size(y,1)/2,floor(size(y,2)/2),0]);
