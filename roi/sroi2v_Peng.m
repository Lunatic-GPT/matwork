suma = '/export/res/retino/segmentation/JiaJie/SUMA/';
%suma = '/mnt/windrive/under_shoot/suma_huang/';
anat = 'anat+orig';
strip_skull = true;

if strip_skull
    cmd = sprintf('@SUMA_AlignToExperiment -exp_anat %s -surf_anat %s -strip_skull exp_anat -align_centers',anat,fullfile(suma,'JieJie_SurfVol_ns+orig'));
else
    cmd = sprintf('@SUMA_AlignToExperiment -exp_anat %s -surf_anat %s -align_centers',anat,fullfile(suma,'JiaJie_SurfVol_ns+orig'));
end
%unix(cmd);

p =parameter('convert surface rois to volume');
p = add(p,'string','subject','JiaJie');
p = add(p,'filename','aligned local anatomy','JiaJie_SurfVol_ns_Alnd_Exp+orig');
