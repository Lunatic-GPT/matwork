function recon_ESPIRiT3D_PaFT(fname,par_array,nmaps,eigThresh_k)
...
res=zeros([nro,lMatrix(2),lMatrix(4),nmaps]);
...
tcg=tic;
nIterCG = 5;
im0=ESP'*(FT'*squeeze(Data4));
 %  res(:,i,:,:)= cgL1ESPIRiT(double(Data3), zeros(imSize(1),imSize(3),nmaps), FT,ESP, nIterCG,XOP,0,0,1);

   res(:,:,i,:)= cgL1ESPIRiT(double(Data4), im0, FT,ESP, nIterCG,XOP,0,0,1)*scl(i);
...
fname=unique_name(sprintf('%s_ESPIRiT_%dmaps_PaFT_%d_%d_%s.mat',prefix,nmaps,par_array(1),par_array(end),num2str(eigThresh_k)));
