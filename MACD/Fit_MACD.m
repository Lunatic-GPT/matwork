...
for i=1:nroi
    ...
    rad(i)=res(3); %% in mm
    vim(roi_vessel==i)=v(i);
    radim(roi_vessel==i)=rad(i);
...
save(save_name,'v','rad','vim','radim','cd_fit','data','center','params','exitflag','output','roi_vessel','data_debug');
if isfield(params,'nii')
    nii=params.nii;
    nii.img=cat(4,roi_vessel,vim,radim);
    prefix_save=strtok2(save_name,'.');
    save_untouch_niigz(nii,prefix_save);
end
...
pos=roiCOM(roi2);
roiRad_i=round(mean(p.roiRad./p.voxSize_interp(1)));  % 1 and 2 should be equal
