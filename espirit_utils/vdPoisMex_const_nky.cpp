...
int genVDPoissonSampling(int *, float, float, int, int, float, float, 
			 int *, int, float,int,int,int &,bool &);
long ran_seed;

void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
  /* function [mask] = vdPoisMex(sx,sy,fovx,fovy,accelx,accely,ncal,pp)*/
  int err;
  int i,j,k,l;
  int csmask[MAXACQS];
  float yfov,zfov;
  ...
  int pncal[2];
  pncal[1]=(int) pcentK[1];
  pncal[0]=(int) pcentK[0];
  
  //doellipyz =  *mxGetPr(prhs[7]);
  pp =   *mxGetPr(prhs[7]);
  float scale =  1;
   
  int nky=floor(sky/redfacKy/redfacKz+0.5);
  
  /* Add the point */
    int nneg=(nky/2-pncal[0]/2);
    int nneg2=sky/2-pncal[0]/2;  // one-based
    
    int npos=(nky-1)/2-(pncal[0]-1)/2;
    int npos2=(sky)/2+1+(pncal[0]-1)/2;  // one-based
    
  int nky_neg_pf=int(nneg*0.5);
  int ky_min;
     int niter=0;   
  while (1)
  {
  ...
  err = genVDPoissonSampling(csmask,   yfov ,zfov, sky, skz,  redfacKy*scale, 
			     redfacKz*scale, pncal, false, pp, nky,nky_neg_pf,ky_min,hasVoid  );

  
      printf("Scale = %3.2f err= %d\n",scale,err);
  ...
  plhs[0] = mxCreateDoubleMatrix(skz,sky-ky_min,mxREAL);
  ...
  for (i=0;i < skz; i++)
    for (j=ky_min;j<sky;j++)
        
	mask[(j-ky_min)*skz+i] = (double)csmask[j*skz+i];
  
}
    

int genVDPoissonSampling(int *acqmask, float fovy, float fovz, int sky, 
			 int skz, float ry, float rz, int* pncal, 
			 int cutcorners, float pp, int nky,int nky_neg_pf,int &min_ky,bool &hasVoid) 
{
    float grid_max;
...
if (nky_neg_pf>nneg){
     sprintf(tmpstr, "nky_neg_pf>nneg\n");
    epic_error(0, tmpstr, 0, 0);
    if (mask) free(mask);
    if (automask) free(automask);
    if (R) free(R);
  ...
  for (i = nneg2; i < npos2; i++) {
    for (j =skz/2-pncal[1]/2; j < (skz)/2+1+(pncal[1]-1)/2; j++) {
      int idx = i*skz + j;
    //  if (automask[idx]) acqmask[idx] = 1;
      acqmask[idx]=1;
    } 
  }
  ...
  /* Generate the two point; one on each side of the ky center */
  ran0(&ran_seed);
  
    for (i=0;i<2;i++)
    { 
  if (i==0)
      pW = nneg2*ran0(&ran_seed);
  else
      pW=npos2+(sky-npos2)*ran0(&ran_seed);
  ...
  /* And use it */
  acqmask[(int)(floor)(pW) *skz + (int)floor( pH)] = mask[(int)floor(pW) *skz + (int)floor(pH)];
    }
  ...
  do {
   ...
   // if(!in_neighborhood && qW<nneg2)
   //         printf("matSum = %d\n" , matSum(acqmask, (int) qH,skz,skz*nneg2-1 ));
    
            
            
 if (!in_neighborhood && ((qW<nneg2  && matSum(acqmask, (int) qH,skz,skz*nneg2-1 )<nneg) ||(qW>=npos2  && matSum(acqmask, (int) qH+npos2*skz,skz,skz*sky-1 )<npos))) 
    {	
	  /* Put the point in the queue */
  ...
  int nSample;
   for (i=0;i<sky-ceil(grid_max*2);i++)
     {
         for (j=0;j<skz-ceil(grid_max*2);j++)
          {
             nSample=0;     
             for (int k=0;k<ceil(grid_max*2);k++)
                 for (int l=0;l<ceil(grid_max*2);l++)
                     nSample+=acqmask[(i+k)*skz+j+l];
 ...
 // setEqualLines(acqmask,skz,sky);
  ...
  min_ky=0;
   for (i=0;i<skz;i++)
   {
       ...
       for (j=nneg2-1;j>=0;j--)
       {
           nSampleNeg+=acqmask[j*skz+i];
           if (nSampleNeg==nky_neg_pf)
           {
               if(j>min_ky) min_ky=j;
               break;
           }
       }   
   }
  ...
  for (j = nneg2 - 1; j >= 0; j--)
  {
	  int nSample = matSum(acqmask, j*skz, nneg2*skz - 1);
	  if (nSample >= skz*nky_neg_pf)
	  {
		  min_ky = j;
		  break;
	  }
   ...
   printf("nky_neg_pf = %d, nneg = %d, nneg2 = %d, npos = %d, npos2 = %d;min_ky = %d\n", nky_neg_pf,nneg, nneg2, npos, npos2, min_ky);

    int src_dim[]={skz,sky};
  int dest_dim[]={skz,nneg2-min_ky};//
  int src_pos[]={0,min_ky,skz,nneg2-min_ky};
  int dest_pos[]={0,0};
  
  int *masktmp=(int *)malloc(skz*(nneg2-min_ky)*sizeof(int));
  
      subMat(acqmask,masktmp,src_dim,dest_dim,src_pos,dest_pos);

      setEqualLines(masktmp,skz,nneg2-min_ky,nky_neg_pf);
  ...
  int *masktmp2=(int *)malloc(skz*(sky-npos2)*sizeof(int));
      src_pos[1]=npos2;
      src_pos[3]=sky-npos2;
      dest_pos[1]=0;
      dest_dim[1]=sky-npos2;
      
      subMat(acqmask,masktmp2,src_dim,dest_dim,src_pos,dest_pos);
      
      
      setEqualLines(masktmp2,skz,sky-npos2,npos);
      
      dest_pos[1]=npos2;
      src_pos[1]=0;
      
      subMat(masktmp2,acqmask,dest_dim,src_dim,src_pos,dest_pos);
