...
kData = zpad(kData,[sx+kSize(1)-1, sy+kSize(2)-1,nCoil,rep]);
...
for y = 1:sy
	...
	%	waitbar((x + (y-1)*sx)/sx/sy,H);
        ...
        tmp=reshape(tmp,length(tmp(:))/rep,rep);
        
		if pat(idxy) | sum(pat)==0  % matrix true only when all are zero
			res(x,y,:) = tmp(idxy,:);
        else
...
%               time_kernel=time_kernel+toc; 
				%disp('add another key');size(KEY,2)
			else
				kernel = LIST(:,idx);
            end
            
			res(x,y,:) = transpose(kernel(:))*tmp;
