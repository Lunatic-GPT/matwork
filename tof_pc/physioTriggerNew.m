...
for i=1:size(interpPeaks,2)
    ...
    ii_rm_tmp=find(i_new>i_new(ii(1)) & i_new<i_new(ii(2)));
    ii_rm=[ii_rm,ii_rm_tmp(:)'];
end
...
i_Peaks2MON=ind_match(t,Peaks2MON);
i_new=[i_orig;i_Peaks2MON];
...
i_new=sort([i_new;i_add(:)]);
i_new=setdiff(i_new,i_Peaks2MON);
...
ind_tmp=find(abs(interval)<minInterval & i_orig~=i_real(i));
