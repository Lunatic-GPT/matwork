...
%[cAH, baAH] = ConfigAxes(fig);  % commented out by xz

% Correlation plot
stats = CalcCorrelationStats(data, params);
% PlotCorrelation(cAH, data, params);  %commented out by xz
cAH=gca; % added by xz
params = FormatPlotAxes(cAH, data, params);

% DisplayCorrelationStats(cAH, params, stats); % commented out by xz
baAH=gca;  %added by xz
...
if ischar(params.axesLimits)
	...
	%	params.axesLimits(2) = max( max(data.maskedSet1), max(data.maskedSet2) );
    	params.axesLimits = [min( min(data.maskedSet1), min(data.maskedSet2) ),max( max(data.maskedSet1), max(data.maskedSet2) )];
...
switch upper(params.baStatsMode)
	case {'NORMAL','GAUSSIAN'}
		...
		plot(a(1:2),stats.differenceMean-stats.rpc*[1 1],':k')
		%text(a(2),stats.differenceMean+stats.rpc,
		%[mynum2str(stats.differenceMean+stats.rpc,2) '
		%(+1.96SD)'],'HorizontalAlignment','left','VerticalAlignment','middle','fontsize',fontsize);
		%%commented out by xz
