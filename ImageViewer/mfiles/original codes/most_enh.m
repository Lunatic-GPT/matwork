function most_enh(subject, number)
% combined_analysis combines analyze_DTI_DCE_ROI with detailed_DTI_analysis
% and includes the consideration of the different time points.


%% THE CUT-OFF VALUES

col = -28.7; %-18.37 -> 50%; % cut-off value on the "left side" (standard: -30 and 30, resp.)
cor = 36; %+25.67 -> 50%; % cut-off value on the "right side"
% cut_off_enh = 0.95; 
%cut_off_enh = 0.9;
%%

frage = input('Do you want to display the wash-in (1) or wash-out (2)?');
close all

save_part = '/temp/'; % CHANGE THIS FOR NEW METHODS!
...
DCEimage_tissue_vec = zeros(6,30*512*512);
DCEimage_tissue2_vec = zeros(6,30*512*512);
DCEimage_lesion_vec = zeros(6,30*512*512);
results = cell(1); % result - cell

relative_change_lesion_vec2 = [];
relative_change_lesion_vec3 = [];
relative_change_lesion_vec4 = [];
relative_change_lesion_vec5 = [];
relative_change_lesion_vec6 = [];

relative_change_tissue1_vec2 = [];
relative_change_tissue1_vec3 = [];
relative_change_tissue1_vec4 = [];
relative_change_tissue1_vec5 = [];
relative_change_tissue1_vec6 = [];

relative_change_tissue2_vec2 = [];
relative_change_tissue2_vec3 = [];
relative_change_tissue2_vec4 = [];
relative_change_tissue2_vec5 = [];
relative_change_tissue2_vec6 = [];

relative_change_wo_vec2 = [];
relative_change_wo_vec3 = [];
relative_change_wo_vec4 = [];
relative_change_wo_vec5 = [];
relative_change_wo_vec6 = [];

relative_change_plat_vec2 = [];
relative_change_plat_vec3 = [];
relative_change_plat_vec4 = [];
relative_change_plat_vec5 = [];
relative_change_plat_vec6 = [];

relative_change_pers_vec2 = [];
relative_change_pers_vec3 = [];
relative_change_pers_vec4 = [];
relative_change_pers_vec5 = [];
relative_change_pers_vec6 = [];
...
%     delta5 = time_points_array(array_line,5);
    ...
    first_slice = time_points_array(array_line,9);
...
wi_3D = zeros(512,512,nslices);
wo_3D = zeros(512,512,nslices);
% most_enh_vol = zeros(512,512,nslices);
enh_wo = [];
...
%         user_entry_slice = input('Assign the slice number of the DCE series of interest: ');
    ...
    save_folder = [save_folder_basic save_part 'slice' user_entry_slice_str];
    ...
    for i=1:6 % six DCE images
        ...
        file = [DCE_image_dir '/shifted_' DCE_number];
        load(file,'image');
        DCE(:,:,i) = image;
    end
    
    file = [DCE_image_dir '/wash_in_slice' user_entry_slice_str];
    load(file,'image');
    washy = image;
    test = washy.*inner_BW_large;
    if slice == 1
my_axis = [x1 x1+x2 y1 y1+y2];
    end
    if frage == 1
        figure; imshow(test,[1 3]); colormap(jet); axis(my_axis);
    end
    %     
...
%     wi_degree = temp;
%     inner_logical = logical(inner_BW_large);
%     temp2 = new_image.*tissue_BW_large;
%     temp3 = new_image.*tissue2_BW_large;
%     wi_lesion = [wi_lesion temp(inner_logical)'];
...
%     if slice == nslices
%         cube = ones(3,3,3);
%         most_enh_vol = imfilter(wi_3D, cube);
%         [D_row D_col2]=find(most_enh_vol==max(max(max(most_enh_vol)))); % find most enhancing 3x3 cube
%         D_col = mod(D_col2,512);
%         D_height = ((D_col2-D_col)/512)+1;
%     end
...
% 
% 
%     wash_in_ROI_vec = [wash_in_ROI_vec wash_in_ROI(inner_logical)'];
%     if slice == nslices
...
% 
%         %% NEW!!! RELATIVE CHANGE VALUES 
%     for i=2:6
%         temp_image = DCE(:,:,i);
%         relative_change = (temp_image-DCE_ref)./DCE_ref;
%         
%         relative_change_lesion = relative_change.*DCE_inner;
%         relative_change_lesion_single_vec = relative_change_lesion(:)';
%         s = ['relative_change_lesion_vec' int2str(i) '= [relative_change_lesion_vec' int2str(i) ' relative_change_lesion_single_vec];'];
%         eval(s);
%         s = ['temp = relative_change_lesion_vec' int2str(i) ';'];
%         eval(s);
%         if slice == nslices
%             relative_change_lesion_mean(i) = mean(temp(temp~=0));
%             relative_change_lesion_std(i) = std(temp(temp~=0));
%         end
% 
%         relative_change_tissue2 = relative_change.*DCE_tissue2;
%         relative_change_tissue2_single_vec = relative_change_tissue2(:)';
%         s = ['relative_change_tissue2_vec' int2str(i) '= [relative_change_tissue2_vec' int2str(i) ' relative_change_tissue2_single_vec];'];
%         eval(s);
%         s = ['temp = relative_change_tissue2_vec' int2str(i) ';'];
%         eval(s);
%         if slice == nslices
%             relative_change_tissue2_mean(i) = mean(temp(temp~=0));
%             relative_change_tissue2_std(i) = std(temp(temp~=0));
%         end
% 
%         relative_change_tissue1 = relative_change.*DCE_tissue;
%         relative_change_tissue1_single_vec = relative_change_tissue1(:)';
%         s = ['relative_change_tissue1_vec' int2str(i) '= [relative_change_tissue1_vec' int2str(i) ' relative_change_tissue1_single_vec];'];
%         eval(s);
%         s = ['temp = relative_change_tissue1_vec' int2str(i) ';'];
%         eval(s);
%         if slice == nslices
%             relative_change_tissue1_mean(i) = mean(temp(temp~=0));
%             relative_change_tissue1_std(i) = std(temp(temp~=0));
%         end
%     end
%         
%     %% END RELATIVE CHANGE VALUES
%     
%     
%     
%     
    ...
    load(file,'image');
    wash_out = image;
    test = wash_out.*inner_BW_large;
    if frage == 2
        figure; imshow(test); colormap(jet); axis(my_axis);
        caxis([-90 90])
    end
...
%     wash_out_ROI_vec = [wash_out_ROI_vec wash_out_ROI(inner_logical)'];
%     if slice == nslices
...
% 
%     
%     %% 3D most enhancing volume
% %     wo_3D(:,:,slice) = wash_out_ROI;
% %     if slice == nslices
% %        for ii = -1:1
% %            for jj = -1:1
% %                for kk = -1:1
% %                    enh_wo = [enh_wo wo_3D(D_row+ii,D_col+jj,D_height+kk)];
% %                end
% %            end
% %        end
% % %        SID
...
%     slope_angle = (180-wi_degree+wash_out_ROI).*DCE_inner;
%     slope = [slope slope_angle(:)];    
%     if slice == nslices
%         slope_mean = mean(slope(slope~=0));
%     end
%     
%     
...
% 
% %% NEW relative change for the areas...
% results = [results 0 'relative change lesion image 1-1 mean' relative_change_lesion_mean(2) 'relative change lesion image 2-1 mean' relative_change_lesion_mean(3) 'relative change lesion image 3-1 mean' relative_change_lesion_mean(4) 'relative change lesion image 4-1 mean' relative_change_lesion_mean(5) 'relative change lesion image 5-1 mean' relative_change_lesion_mean(6) 'relative change lesion image 6-1 mean'];
% results = [results 0 'relative change lesion image 1-1 std' relative_change_lesion_std(2) 'relative change lesion image 2-1 std' relative_change_lesion_std(3) 'relative change lesion image 3-1 std' relative_change_lesion_std(4) 'relative change lesion image 4-1 std' relative_change_lesion_std(5) 'relative change lesion image 5-1 std' relative_change_lesion_std(6) 'relative change lesion image 6-1 std'];
% results = [results 0 'relative change tissue1 image 1-1 mean' relative_change_tissue1_mean(2) 'relative change tissue1 image 2-1 mean' relative_change_tissue1_mean(3) 'relative change tissue1 image 3-1 mean' relative_change_tissue1_mean(4) 'relative change tissue1 image 4-1 mean' relative_change_tissue1_mean(5) 'relative change tissue1 image 5-1 mean' relative_change_tissue1_mean(6) 'relative change tissue1 image 6-1 mean'];
% results = [results 0 'relative change tissue1 image 1-1 std' relative_change_tissue1_std(2) 'relative change tissue1 image 2-1 std' relative_change_tissue1_std(3) 'relative change tissue1 image 3-1 std' relative_change_tissue1_std(4) 'relative change tissue1 image 4-1 std' relative_change_tissue1_std(5) 'relative change tissue1 image 5-1 std' relative_change_tissue1_std(6) 'relative change tissue1 image 6-1 std'];
% results = [results 0 'relative change tissue2 image 1-1 mean' relative_change_tissue2_mean(2) 'relative change tissue2 image 2-1 mean' relative_change_tissue2_mean(3) 'relative change tissue2 image 3-1 mean' relative_change_tissue2_mean(4) 'relative change tissue2 image 4-1 mean' relative_change_tissue2_mean(5) 'relative change tissue2 image 5-1 mean' relative_change_tissue2_mean(6) 'relative change tissue2 image 6-1 mean'];
% results = [results 0 'relative change tissue2 image 1-1 std' relative_change_tissue2_std(2) 'relative change tissue2 image 2-1 std' relative_change_tissue2_std(3) 'relative change tissue2 image 3-1 std' relative_change_tissue2_std(4) 'relative change tissue2 image 4-1 std' relative_change_tissue2_std(5) 'relative change tissue2 image 5-1 std' relative_change_tissue2_std(6) 'relative change tissue2 image 6-1 std'];
% %%
% 
...
% results = [results slope_mean 'slope angle'];
...
%     end
%     
%     %% NEW WASH-IN (in degrees): created 6/28/2008, for determination of
%     %% most enhancing area
%     image1 = DCE(:,:,1);
%     image2 = DCE(:,:,2);
%     new_image = image2-image1;
...
%     
%     
%     tempi = imfilter(temp,[0 1 0; 1 1 1; 0 1 0]);
%     max_enh = max(tempi(:));
%     [row, column] = find(tempi==max_enh);
%     row = row(1);
%     column = column(1);
%     most_enh_mask = zeros(512,512);   
%     most_enh_mask(row,column+1)=1;
%     most_enh_mask(row,column)=1;
%     most_enh_mask(row,column-1)=1;
%     most_enh_mask(row+1,column)=1;
%     most_enh_mask(row-1,column)=1;
% 
%     BW_enh = most_enh_mask;
%     BW_wo = BW_enh; % THE 'WASH-OUT-AREA' is now the most enhancing area...
%     BW_wo_area = sum(sum(BW_wo==1));
% 
% %     temp2 = temp(:);
% %     temp2 = temp2(temp2~=0);
% %     %min_enh = min(temp2);
% %     max_enh = max(temp2);
% %     %threshold = cut_off_enh*(max_enh-min_enh)+min_enh; % selects the most enhancing area (1std-cutoff)
% %     %BW_enh = roicolor(temp, threshold,90);
% %     [row,column] = find(temp==max_enh);
% %     row = row(1);
% %     column = column(1);
% %     most_enh_mask = zeros(512,512);
% %     for m_i=-1:1
% %        for m_j = -1:1        
% %             most_enh_mask(row+m_i,column+m_j)=1;
% %        end
% %     end
% %     
% %     temp_enh = most_enh_mask.*temp;
% %     most_enh_mask = zeros(512,512);
% %     most_enh_mask(row,column)=1;
% %     sorti = sort(temp_enh(:),'descend');
% %     max_b = sorti(2);
% %     max_c = sorti(3);
% %     [row,column] = find(temp==max_b);
% %     if max_b == max_enh
% %         row = row(2);
% %         column = column(2);
% %     else
...
% %     most_enh_mask(row,column)=1;
% %     [row,column] = find(temp==max_b);
% %     if max_c == max_b && max_b == max_enh
% %         row = row(3);
% %         column = column(3);
% %     elseif max_c == max_b && max_b ~= max_enh
% %         row = row(2);
% %         column = column(2);
% %     else
% %         row = row(1);
...
% %     
%     wi_enh = temp.*BW_enh;
%     wo_enh = wash_out.*BW_enh;
%     temp = (180-wi_enh+wo_enh).*BW_enh;
%     wi_enh_vec = [wi_enh_vec temp(:)];
% 
%     if slice == nslices
%         wi_enh_mean = mean(wi_enh_vec(wi_enh_vec~=0));
%     end
% 
%     DCE_ref = double(DCE(:,:,1));
%     DCE_ref(DCE_ref==0)=1; % setting zeros to one, we will forget these pixels later by using the 'no_zero_mask', just set so that the division by DCE_ref is possible
...
%         relative_change = (temp_image-DCE_ref)./DCE_ref;
%         
%         relative_change_wo = relative_change.*BW_wo;
%         relative_change_wo_single_vec = relative_change_wo(:)';
%         s = ['relative_change_wo_vec' int2str(i) '= [relative_change_wo_vec' int2str(i) ' relative_change_wo_single_vec];'];
%         eval(s);
%         s = ['temp = relative_change_wo_vec' int2str(i) ';'];
%         eval(s);
%         if slice == nslices
...
%         end
% 
%         relative_change_pers = relative_change.*BW_pers;
%         relative_change_pers_single_vec = relative_change_pers(:)';
%         s = ['relative_change_pers_vec' int2str(i) '= [relative_change_pers_vec' int2str(i) ' relative_change_pers_single_vec];'];
%         eval(s);
%         s = ['temp = relative_change_pers_vec' int2str(i) ';'];
%         eval(s);
%         if slice == nslices
...
%         end
% 
%         relative_change_plat = relative_change.*BW_plat;
%         relative_change_plat_single_vec = relative_change_plat(:)';
%         s = ['relative_change_plat_vec' int2str(i) '= [relative_change_plat_vec' int2str(i) ' relative_change_plat_single_vec];'];
%         eval(s);
%         s = ['temp = relative_change_plat_vec' int2str(i) ';'];
%         eval(s);
%         if slice == nslices
...
% results = [results wash_out_wo_mean 'wash-out wo mean' wash_out_wo_std 'wash-out wo std' wash_out_pers_mean 'wash-out pers mean' wash_out_pers_std 'wash-out pers std' wash_out_plat_mean 'wash-out plat mean' wash_out_plat_std 'wash-out plat std'];
...
% 
% %% NEW relative change for the areas...
% results = [results 0 'relative change wo image 1-1 mean' relative_change_wo_mean(2) 'relative change wo image 2-1 mean' relative_change_wo_mean(3) 'relative change wo image 3-1 mean' relative_change_wo_mean(4) 'relative change wo image 4-1 mean' relative_change_wo_mean(5) 'relative change wo image 5-1 mean' relative_change_wo_mean(6) 'relative change wo image 6-1 mean'];
% results = [results 0 'relative change wo image 1-1 std' relative_change_wo_std(2) 'relative change wo image 2-1 std' relative_change_wo_std(3) 'relative change wo image 3-1 std' relative_change_wo_std(4) 'relative change wo image 4-1 std' relative_change_wo_std(5) 'relative change wo image 5-1 std' relative_change_wo_std(6) 'relative change wo image 6-1 std'];
% results = [results 0 'relative change plat image 1-1 mean' relative_change_plat_mean(2) 'relative change plat image 2-1 mean' relative_change_plat_mean(3) 'relative change plat image 3-1 mean' relative_change_plat_mean(4) 'relative change plat image 4-1 mean' relative_change_plat_mean(5) 'relative change plat image 5-1 mean' relative_change_plat_mean(6) 'relative change plat image 6-1 mean'];
% results = [results 0 'relative change plat image 1-1 std' relative_change_plat_std(2) 'relative change plat image 2-1 std' relative_change_plat_std(3) 'relative change plat image 3-1 std' relative_change_plat_std(4) 'relative change plat image 4-1 std' relative_change_plat_std(5) 'relative change plat image 5-1 std' relative_change_plat_std(6) 'relative change plat image 6-1 std'];
% results = [results 0 'relative change pers image 1-1 mean' relative_change_pers_mean(2) 'relative change pers image 2-1 mean' relative_change_pers_mean(3) 'relative change pers image 3-1 mean' relative_change_pers_mean(4) 'relative change pers image 4-1 mean' relative_change_pers_mean(5) 'relative change pers image 5-1 mean' relative_change_pers_mean(6) 'relative change pers image 6-1 mean'];
% results = [results 0 'relative change pers image 1-1 std' relative_change_pers_std(2) 'relative change pers image 2-1 std' relative_change_pers_std(3) 'relative change pers image 3-1 std' relative_change_pers_std(4) 'relative change pers image 4-1 std' relative_change_pers_std(5) 'relative change pers image 5-1 std' relative_change_pers_std(6) 'relative change pers image 6-1 std'];
% %%
% 
% results = [results wi_enh_mean 'slope angle most enhanced area'];
% 
% results2 = cell(1);
% for i =1:((size(results,2)-1)/2); results2(i,1)=results(1,2*i);end
% for i =1:((size(results,2)-1)/2); results2(i,2)=results(1,1+2*i);end
...
% %savename = [save_folder '/wash_in_vector_degree.mat'];
% %save(savename, 'wi_lesion');
% 
% %savename = [save_folder '/wash_in_vector_uptake.mat'];
% %save(savename, 'wash_in_ROI_vec');
% 
% 
% close all