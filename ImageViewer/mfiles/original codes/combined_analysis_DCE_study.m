function combined_analysis_DCE_study(subject, number)
...
col = 0; %-18.37 -> 50%; % cut-off value on the "left side" (standard: -30 and 30, resp.)
cor = 0; %+25.67 -> 50%; % cut-off value on the "right side"

%%

save_part = '/research_results_detailed_two_areas/'; % CHANGE THIS FOR NEW METHODS!
...
%     saveas(gcf,savename)
    ...
    for i=1:6
        ...
        %% END WEIGHTED DIFFERENCE CALCULATION FOR TIME_SERIES_WDIFF
            ...
            %tempi = DCEimage_tissue(DCEimage_tissue~=0);
            %temp_size = size(DCEimage_tissue_vec,2);
            %temp_size2 = size(tempi,2);
            %DCEimage_tissue_vec(i,temp_size+1) = DCEimage_tissue(:)';
            if slice == nslices
