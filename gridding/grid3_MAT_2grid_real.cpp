 ...
 * \n\
 *  crds: coordinates for input data, fastest varying dimension is length 3 \n\
 *          trajectory coordinate points are typically scaled between -0.5 to 0.5 \n\
 * \n\
 *  newcrds: coordinates for output data, fastest varying dimension is length 3 \n\
 * \n\
 *  effMtx: (integer) scalling factor for kernalRadius, kernelRadius = 4/effMtx.\n\
    ...
    */
    ...
    {
        printf("%s",help_txt);
        mexErrMsgTxt("4 required inputs are: data, crds, newcrds, effMtx");
    }
    ...
    {
        printf("%s",help_txt);
        mexErrMsgTxt("Only 1 output arg is returned from grid3_MAT_2grid_real().");
    }
    ...
    unsigned long nd = mxGetNumberOfDimensions(prhs[0]); /* get coordinate dimensions */
    ...
    /* COORDS */
    printf("copy new coords\n");
        assert(prhs[2] != NULL);     /* check existence */
        assert(mxIsDouble(prhs[2])); /* check for type double */
    nd = mxGetNumberOfDimensions(prhs[2]); /* get coordinate dimensions */
        assert( nd > 0 ); /* check for valid array size */
    dims = mxGetDimensions(prhs[2]);
        assert( dims[0] == 3 ); /* make sure the fastest varying dim holds an [x,y,z] triplet */
    dims_l = (unsigned long *) malloc(sizeof(unsigned long) * nd);
    for (i=0;i<nd;i++) dims_l[i] = (unsigned long) dims[i]; 
    dataArray_double *coords2 = new_dataArray_double(nd,dims_l); /* alloc new dataArray_double */
        assert(coords != NULL); /* make sure new mem is allocated */
    memcpy( coords2->data, mxGetPr(prhs[2]), sizeof(double)*(coords2->num_elem) ); /* copy coords */
    free(dims_l);
 
    /* allocate output array */
    nd = mxGetNumberOfDimensions(prhs[2])-1; /* get coordinate dimensions */
    dims = mxGetDimensions(prhs[2]);
    dims_l = (unsigned long *) malloc(sizeof(unsigned long) * nd);
    for (i=0;i<nd;i++) dims_l[i] = (unsigned long) dims[i+1]; 
    ...
    for(i=0;i<kern->num_elem;i++) kern->data[i] = 0.;
...
{    
      ...
      unsigned  long kern_ind;
     if (i%2000==0) 
     {
        printf("%d/%d\n",i,data->num_elem);
        mexEvalString("drawnow;");
     }
	for (unsigned long j=0;j<coords2->num_elem/3;j++)
    { 
    	double x2 = coords2->data[j*3]; 
		double y2 = coords2->data[j*3+1];
		double z2 = coords2->data[j*3+2];
       ...
       double dist_sqr=(x-x2)*(x-x2)+(y-y2)*(y-y2)+(z-z2)*(z-z2);
        ...
        kern_ind=(unsigned long)(dist_sqr*dist_multiplier+0.5);
