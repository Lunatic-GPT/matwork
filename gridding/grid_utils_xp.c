...
#define KERNEL_WIDTH					8.0// was 5.0  
...
#define getas1c(_in, _i) (*(complex<double> *) ((_in)->data + (_i * 2)))		/* returns a complex double */
#define get3c(_in, _i, _j, _k) (*(complex<double> *) (((_in)->data) + \
            ...
            double *windowLength_p ) 		/* coord scale */
{
    ...
    int width, max_x,max_y,max_z;
    double  kernelRadius,kernelRadius_sqr,kernelRadius_invSqr,dist_multiplier, dist_sqr, width_inv,windowLength;
    ...
    /* get rid of pointers */
	radiusFOVproduct = *radiusFOVproduct_p, windowLength = *windowLength_p;

    /* grid size this has already been multiplied by osf */
	width = *gridsize; /* square matrix */
    max_x = out_list[*cur_thread]->dimensions[1];
    max_y = out_list[*cur_thread]->dimensions[2];
    max_z = out_list[*cur_thread]->dimensions[3];
    
    /* Calculate kernel radius in units of pixels and 1/FOV 
     *      rfp should also be appropriately scaled by oversampling factor*/
	 kernelRadius           = radiusFOVproduct / (double)width;
	 kernelRadius_sqr       = kernelRadius * kernelRadius;
	 kernelRadius_invSqr    = 1.0 / (kernelRadius * kernelRadius);

    /* access kernel table from kr */
	 dist_multiplier, dist_sqr;
	 width_inv = 1.0 / width;
    ...
    /* zero output field */
    for(ii=0;ii<out_list[*cur_thread]->num_elem;ii++)
        out_list[*cur_thread]->data[ii] = 0.;

    /* get center pt for current partition */
    center_x = int(width/2+0.5);
    center_y = int(width/2+0.5);
    center_z = int(width/2+0.5);
            ...
            for (j=jmin; j<=jmax; j++)	
            {
                ...
                if (dz_sqr_PLUS_dy_sqr < kernelRadius_sqr)	
                {
                    for (i=imin; i<=imax; i++)	
                    {
                        ...
                        {
                            /* get kernel value */
                            complex<double> val = kernel_in->data[(int)(dist_sqr * dist_multiplier+0.5)];
