function Peak_Dip_Sort2Ave2(bname,nTR_pk, nTR_dp,bl,nTR_trial,prefix)
%Peak_Dip_Sort2Ave2(bname,nTR_pk,nTR_dip,bl,nTR_trial,prefix)
% peak (dip) positions are determined automatically by finding the
% maximum (minimum) by summing over the neighboring nTR_pk (nTR_dip) time points. 
...
for i=1:length(ind_nbl)-nTR_dp+1
    i1 = ind_nbl(i);
  ts_dp(:,:,:,i) = mean(ts_mn(:,:,:,i1:i1+nTR_dp-1),4);
end

data = zeros([sz(1:3),12]);

[data(:,:,:,1),ipk] = max(ts_pk,[],4);
[data(:,:,:,5),idp] = min(ts_dp,[],4);
