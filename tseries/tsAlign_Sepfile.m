function tsAlign_Sepfile(p)
% tsAlign(p)
% save each trial as seperate files to avoid "out of memory" error
  ...
  for i=1:size(sti_seq,1)
    ...
    for j=1:size(sti_seq,2)
      ...
      if dim3
       ...
       opt.Prefix = sprintf('%s_p%d_%d',out_name,itrl,t_count);
       opt.OverWrite = 'y';
       if ~exist([opt.Prefix,'+orig.HEAD'],'file')
         WriteBrik(brikdata,info,opt);
         disp('saving the aligned time series');
       else
           disp([opt.Prefix,'+orig.HEAD',' exists']);
       end
