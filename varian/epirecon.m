...
% 8/6/'10 +total_tr=tr*nseg if not exist
% 8/31/'10 +imresize in image domain
% 9/13/'10 *rewrite pss output
% 9/21/'10 *total_tr unit = ms, tr unit = sec +option to have both nav mag and phase correction
% 9/22/'10 +output singlegrad, readtype and readtab
% 9/28/'10 +output exp time
% 9/30/'10 +output orient, seqfil, now
% 10/7/'10 +shift corr_pro 
% 1/23/'11 *remove fopen text mode
% 1/24/'11 + readtype=la linear read-out interpolation
%          *don't correct first spike
% 3/25/'11 *output sdt by default
%
% TODO: linear ramp sampling re-griding, measure center of mass
...
while 1
    ...
    if (line == -1)
        ...
        elseif (strcmp(name, 'ne')) % Number of echoes to be acquired (P)
            line=(sscanf(fgets(fp),'%d'))';
            ne=line(2:end);
        elseif (strcmp(name, 'truenp')) || (strcmp(name, 'np_proc')) % Number of data point aftet interpolation (P)
            line=(sscanf(fgets(fp),'%d'))';
            truenp=line(2:end);
		...
		elseif (strcmp(name, 'readtype')) % Readout type (lf or la or na)
            line = fgets(fp);
            [cnt, parm] = strtok(line);
            readtype = strtok(strjust(parm(1:end-1),'left'),'""');
		elseif (strcmp(name, 'readtab')) % Readout gradient shape in table
            line = fgets(fp);
            [cnt, parm] = strtok(line);
            readtab = strtok(strjust(parm(1:end-1),'left'),'""');
		elseif (strcmp(name, 'singlegrad')) % Using hardloop or softloop for readout (Y or N)
            line = fgets(fp);
            [cnt, parm] = strtok(line);
            singlegrad = strtok(strjust(parm(1:end-1),'left'),'""');
		elseif (strcmp(name, 'time_run')) % Exp starting date and time
            line = fgets(fp);
            [cnt, parm] = strtok(line);
            time_run = datenum(strtok(strjust(parm(1:end-1),'left'),'""'),'yyyymmddTHHMMSS');
		elseif (strcmp(name, 'time_complete')) % Exp finishing date and time
            line = fgets(fp);
            [cnt, parm] = strtok(line);
            time_complete = datenum(strtok(strjust(parm(1:end-1),'left'),'""'),'yyyymmddTHHMMSS');
		elseif (strcmp(name, 'orient')) % Slice orientation (combination of x, y, z, n or sag, trans, cor)
            line = fgets(fp);
            [cnt, parm] = strtok(line);
            orient = strtok(strjust(parm(1:end-1),'left'),'""');
		elseif (strcmp(name, 'seqfil')) % Pulse sequence used
            line = fgets(fp);
            [cnt, parm] = strtok(line);
            seqfil = strtok(strjust(parm(1:end-1),'left'),'""');
        elseif (strcmp(name, 'interpolation')) % Using linear interpolation for readout (Y or N)
            line = fgets(fp);
            [cnt, parm] = strtok(line);
            interpolation = strtok(strjust(parm(1:end-1),'left'),'""');
        end
    end
end
...
if( exist([ '~/labhome/vnmrsys/tablib/' petable ], 'file') )
    pathpe = [ '~/labhome/vnmrsys/tablib/' petable ];
elseif( exist( [ './' petable ], 'file') )
    pathpe = [ './' petable ];
else
    error('table: %s does not exist in ~/vnmrsys/tablib/ or ./', petable)
end

if arraydim<2
    error('arraydim<2!')
elseif arraydim<ref_pos+1
	error('no referance scan! ref_pos=%g',ref_pos)
end

if ~exist('total_tr','var')
    total_tr=tr*nseg*1000;
end
...
% if strcmp(readtype,'la')
%     error('epirecon does not support readout type of linear all')
% end

if ~strcmp(singlegrad,'y') && strcmp(readtype,'la')
    if( exist([ '~/vnmrsys/shapelib/' readtab '.GRD' ], 'file') )
        pathro = [ '~/vnmrsys/shapelib/' readtab '.GRD'];
    elseif( exist( [ './' readtab '.GRD'], 'file') )
        pathro = [ './' readtab '.GRD'];
    else
        error('table: %s.GRD does not exist in ~/vnmrsys/tablib/ or ./', readtab)
    end
end
...
fprintf('pro/ppe/pss= %gmm /%gmm /', pro*10, ppe*10);
for n=1:length(pss)
	if n==1
		fprintf('%g', pss(n)*10);
	else
...
fprintf('singlegrad=%s, readtype=%s', singlegrad, readtype);
if strcmp(singlegrad,'y')
	fprintf('\n');
else
	fprintf(', readtab=%s\n', readtab);
end

fprintf('ss=%g, ref_pos=%g, orient=%s, seqfil= %s\n',ss,ref_pos, orient, seqfil);

fprintf('start: %s, finish: %s\n', datestr(time_run,31), datestr(time_complete,31));
...
%%%%%%%%%%%%%% Linear Read-out Interpolation  %%%%%%%%%%%%%%

if ~strcmp(singlegrad,'y') && strcmp(readtype,'la')
    if interpolation=='y'
        fprintf('linear interpolating in read-out direction...')
        inPtr = fopen(pathro, 'r', endian);
        tmp_cell = textscan(inPtr,'%d16 %u8');
        fclose(inPtr);
        ...
        rdtable(sum(durat)+1)=0;
        n=2;
        for m=1:length(durat)
            if durat(m)>0
                rdtable(n:n+durat(m)-1)=grad(m);
                n=n+durat(m);
            end
        end

        rdtable_intp=interp1q((0:length(rdtable)-1)',rdtable',((0:2/np:1)*(length(rdtable)-1))');
        
        accum=zeros(size(rdtable_intp));
        for m=2:length(rdtable_intp)
            accum(m)=rdtable_intp(m)+accum(m-1);
        end
        intg_la=accum(end)*2/truenp;
        clear rdtable_intp
        raw_new=zeros(truenp/2,size(raw,2),size(raw,3),size(raw,4));
        ...
        for m=1:arraydim
            for n=1:ns
                for p=1:nv
                    raw_new(:,p,n,m)=interp1q(accum,raw(:,p,n,m),(intg_la:intg_la:accum(end))');
                end
            end
        end
        ...
        fprintf('revmoving extra points from linear ramp sampling...')
        raw=raw(floor(1+(np-truenp)/4:(np+truenp)/4),:,:,:);
    end
    np=truenp;
    fprintf('done\n')
end
...
%%%%%%%%%%%%%% Phase Encoding Re-ordering %%%%%%%%%%%%%%

fprintf('converting data in table order(%s) to linear order...',petable)

inPtr = fopen(pathpe, 'r', endian);
inStr = fgets(inPtr);
...
% Pfeuffer J. MRM 47:344-353 (2002)
% not working
...
%    if m~=resp_pos
%        for n=1:ns
%            del_omega=angle(raw(np/4+1,nnv/2+1,n,m)/raw(np/4+1,nnv/2+1,n,resp_pos));
            
%            exp(-j*del_omega*t/te)
...
%fprintf('moving average in time domain for center of k-Space...')
%skip=8;
%resp_pat=[0.27 1 1 1 0.27];
%resp_sum=sum(resp_pat);

%for m=1:ns
%    for n=nnv/2-1:nnv/2
%        for p=np/4-1:np/4
%            raw_tmp=(filter(resp_pat,1,raw(p,n,m,skip+1:end-1)))/resp_sum;
%            raw(p,n,m,skip+1+4:end-1)=raw_tmp(5:end);
%        end
%    end
%end
%fprintf('done\n')
...
corr_pro=0;
if corr_pro ~= 0
    fprintf('shifting %gmm in RO...',corr_pro*10)
    roShift = (exp(-1i*2*pi*corr_pro*([0:np/4-1,-np/4:-1])/lro))'*ones(1,nv);
    for m=1:ar_length
        for n=1:ns
            raw(:,:,n,m) = raw(:,:,n,m).*roShift;
        end
    end
    fprintf('done\n')
    clear roShift
...
%%%%%%%%%%%%%% Non-linear Phase Correction using Calibration Scan %%%%%%%%%%%%%%
% Bruder et al.; MRM 23:311 (1992)
...
if strcmp(nav,'y')
    ...
    for m=1:arraydim
        for n=1:ns
            for p=2:nseg
                raw_nav(:,nnv/nseg*(p-1)+1:nnv/nseg*p,n,m) = raw(:,nnv/nseg*(p-1)+1:nnv/nseg*p,n,m).*(exp(i*angle(raw(:,nnv+1,n,m)./raw(:,nnv+p,n,m)))*ones(1,nnv/nseg));
				%raw_nav(:,nnv/nseg*(p-1)+1:nnv/nseg*p,n,m) = raw(:,nnv/nseg*(p-1)+1:nnv/nseg*p,n,m).*(raw(:,nnv+1,n,m)./raw(:,nnv+p,n,m)*ones(1,nnv/nseg)); %mag and phase corr
            end
        end
    end
    raw = raw_nav;
    fprintf('done\n')
	%raw=raw(:,1:(end-nseg),:,:); % No Nav corr
	%fprintf('FAIL!\n')
    clear raw_nav
end
...
%fprintf('bicubic interpolation in image domain...')
%raw=abs(raw);
%for m=1:siz(3)
    %for n=1:siz(4)
		%raw(:,:,m,n) = imresize(raw(:,:,m,n),[64 64]);
    %end
%end
%fprintf('done\n')
...
fwrite(fid, abs(raw), 'float32'); % dataType
fclose(fid);
...
% Write spr file
fid = fopen([strtok(dirpath,'.'), '.spr'],'w', endian);
