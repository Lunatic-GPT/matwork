...
if ~strcmp(computer,'PCWIN64')
    ...
    elseif exist('/home/xiaopeng/oldvnmrsys','dir');
...
if readPar(fid_dir,'ni')==0 || ~isempty(tabfile) %no tabc in vnmrj
    
    z=tabc(z,fullfile(root,'tablib',tabfile(2:end-1)));
else
    pelist=readPar(fid_dir,'pelist');
    [tmp,ind]=sort(pelist);
    z=z(:,ind,:,:);  
end

np=readPar(fid_prefix,'np');
npe=readPar(fid_prefix,'nv');
ns=readPar(fid_prefix,'ns');

z=reshape(z,[np/2,npe,ns,size(z,3)/ns]);
...
if nargout ==0
 ...
 fprintf('tbgems shaper finished. Image dimensions:%d*%d*%d\n',size(img,1),size(img,2),size(img,3));
     ...
     fprintf('tbgems shaper finished. Image dimensions:%d*%d*%d*%d\n',size(img,1),size(img,2),size(img,3),size(img,4));
 end
 img = '';
end
